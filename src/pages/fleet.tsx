...
export default function Fleet() {
  const [fleetFilter,setFleetFilter] = useState(undefined)
  ...
  const clearFleetFilter = ()=>{
    setFleetFilter(undefined)
  }

  const filterFleet = (fleet:any)=>{
    setFleetFilter(fleet)
  }
   ...
   >
    ...
    className= "  relative z-10 overflow-hidden bg-cover bg-top bg-no-repeat "
        >
         ...
         <FleetData clearFleetFilter={clearFleetFilter} filterFleet={filterFleet}/>
...
/>
 ...
 {visibleMarkers.map((marker, index) => {
  ...
  if(marker && (fleetFilter == undefined ? true: (marker?.data.flight.startsWith(fleetFilter) ? true:false)))
  return(
      <Marker
        key={index}
